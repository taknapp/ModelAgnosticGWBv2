...
Tvals = [7* 24 * 60 * 60,   14* 24 * 60 * 60,   30* 24 * 60 * 60,      
         365.25* 24 * 60 * 60, 365.25* 24 * 60 * 60*2]   
...
for kk, T in enumerate(Tvals): 
    ...
    try:
        ...
        plot_posterior_fits(fit_omega, fit_results_omega, freqs, N_samples, offset=N_offset, num_posteriors=200, label_str = f'{T/(24 * 60 * 60*365.25)} yrs', alpha=0.5)
...
plt.tight_layout()
plt.show()
plt.savefig(f'tsf_posterior_fits_{noise_type}_{sig_type}.pdf')
