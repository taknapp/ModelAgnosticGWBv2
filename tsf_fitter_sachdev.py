...
for kk, T in enumerate(Tvals): 
    ...
    # SAMPLE RJMCMC
    fit_omega, fit_results_omega = sample_Omega(freqs, N_samples, data_obj, N_possible_knots=2)

    pickle.dump([freqs, data, signal, fit_omega, fit_results_omega], open(f'{sig_type}_signal_data_{T/(24 * 60 * 60)}_CE_2.pkl', 'wb'))
...
for kk, T in enumerate(Tvals): 
    ...
    pickle.dump([freqs, data, signal, fit_omega, fit_results_omega], open(f'{sig_type}_signal_data_{T/(24 * 60 * 60)}_CE.pkl', 'wb'))
