...
def generate_data(signal_name, freqs, sigma, BPL_turnover=60):
    ...
    elif signal_name == 'Sachdev1a':
        file_path = 'Yellow_Curve_1a.csv'
    ...
    elif signal_name == 'Sachdev1e':
        file_path = 'Red_Curve_1e.csv'
        data = pd.read_csv(file_path)
        print(data.columns)
        x = data['x']
        y = data[' y']
...
def sample_Omega(freqs, N_samples, data_obj, N_possible_knots=30):
    ...
    fit_omega = FitOmega(data_obj, N_possible_knots, (freqs[0], freqs[-1]), (-13, -3), log_output=True, log_space_xvals=True, min_knots=0)
...
def plot_posterior_fits(fit_omega, fit_results_omega, freqs, N_samples, offset=0, num_posteriors=1000, label_str='', alpha=0.75 ):
    ...
    for ii in range(num_posteriors):
        ...
        #print('fit_func : ', fit_func2)
        #plt.plot(freqs, fit_func2, alpha=1, c='k')
    ...
    #print(*np.percentile(np.array(fit_funcs), [5,95], axis=0))
    plt.fill_between(freqs, *np.percentile(np.array(fit_funcs), [5,95], axis=0), alpha= alpha, label = label_str)
